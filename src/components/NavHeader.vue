<template>
            ...
            <div class="md-content">
              <div class="confirm-tips">
                <div class="error-wrap">   
                  <!-- v-show 和 v-if 差别是show只是hidden 而v-if就完全没有这个标签 -->
                ...
                <ul>
                  ...
                  <li class="regi_form_input noMargin">
                    ...
                    <input type="password" tabindex="2"  name="password" v-model="userPwd" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="Password" @keyup.enter="login"> <!-- 这里keyup是键盘事件，enter是回车键，当按回车键触发login -->
                  </li>
                </ul>
              </div>
        ...
        <!-- 当把遮罩层和 登入弹出框 绑定在一起一个弹出框的时候，有个好处就是可以用一个loginModalFlag=false控制 -->
...
<script>
  ...
  import { mapState } from 'vuex' //mapState是一个对象的结构 vuex返回的是一个大类的对象
  //这个插件就是相当于 通过路由获取后台 加工过信息的一个组件，就相当于连接前后端的一个东西
  export default{
    data(){
      return {
        userName:'',
    ...
    mounted(){ //mounted() 是vue生命周期初始化函数
      this.checkLogin();
      this.getCartCount();
    },
    computed:{  //通过计算属性来时时改变状态
      ...mapState(['nickName','cartCount'])// 三个点...是对object的一个解读(相当于一个展开运算符)，把mapState里面的数组展开成下面注释的形式
        
      //mapstate最终会返回下面那个形式，nickName,cartCount也都是里面是return的方法，返回的也都是值
      /*nickName(){
    ...
    methods:{
        checkLogin(){
          axios.get("/users/checkLogin").then((response)=>{
            ...
            var path = this.$route.pathname; //只有表示路由的时候才能是$route
            if(res.status == '0'){
              //this.nickName = res.result.userName
              this.$store.commit("updateUserInfo",res.result); //从Vuex 里面调用状态，防止往各个navheader里面传值
              this.loginModalFlag = false;
              this.getCartCount();
            }else{  //如果是其他乱七八糟的路由那么固定到/goods 首页
        ...
        login(){
          ...
          }).then((response)=>{ //这里的then()是一个回调函数，就是执行完后台程序，返回结果
            let res = response.data;
            if(res.status == "0"){
              this.errorTip = false; //成功
              this.loginModalFlag = false;
              //to-do
