<template>
  <div> <!-- //nav-header这些子组件都是放在 template 下的第一个div里面的 -->
            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a class="checkbox-btn item-check-btn" v-bind:class="{'check':item.checked=='1'}" @click="editCart('checked',item)">
                      <!-- v-bind:class="{'check':item.checked=='1'}" 这里check是内置的，当恒等为1就是选中，没有就是不选中 -->
                  ...
                  <div class="cart-item-pic">
                    <img v-bind:src="'/static/'+item.productImage" v-bind:alt="item.productName"> <!-- 这个alt是鼠标点到图片显示的文字 -->
                  </div>
                ...
                <div class="cart-tab-2">
                  <div class="item-price">{{item.salePrice | currency('$','1')}}</div> <!-- 如果要传个参数是数字那么也可以用‘’括号括来 -->
                </div>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.productNum*item.salePrice | currency('$','1')}}</div>
                </div>
          ...
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                Item total: <span class="total-price">{{totalPrice | currency('$','1')}}</span> 
                <!-- currency相当于一个函数，需要传一个'$'符号起过滤功能 -->
              </div>
              <div class="btn-wrap">
    ...
    <Modal :mdShow="modalConfirm" @close="closeModal">
      ...
      <div slot="btnGroup">
          ...
          <a href="javascript:;" class="btn btn--m" @click="modalConfirm=false">关闭</a> <!-- router-link是一个超链接 -->
      </div>
    </Modal>
...
<script>
    ...
    //import {currency} from './../util/currency.js' //{currency}是导出的名字
    //过滤器分为全局（定义在main）的和局部的（单个页面的default里面）
    export default{
        data(){
            return{
              cartList:[],
              modalConfirm:false,
              delItem:''
             // productId:'' //把productId全局缓存起来，这样删除的时候能用到
              //checkAllFlag:false  如果要把checkAllFlag这个属性放到computed里面那么这个值就不要在全局定义了，因为 data()里面优先级是最高的如果这里定义了false ，那么checkAlltoggle不起效，不能全部取消，因为本来是false，再点下只能true
            }
        },
        computed:{ //放在 computed{}里面 ，功能就不用每次去渲染了，只要中间有个变量发生变化就会进行时实计算
          checkAllFlag(){ //对checkAllFlag进行时实计算，这样 就不用定义在data中第一了
            return this.checkedCount == this.cartList.length; //this.checkedCount 是一个方法的返回值
          },
          checkedCount(){
            var i = 0;
            this.cartList.forEach((item)=>{ //在cartList里面一个一个去判断
              if(item.checked == '1'){//只要item.checked这个值发生变化，就会自动渲染
                i++;
              }
            })
            return i;
          },
          totalPrice(){ //因为总金额是需要时时监控的，所以要放在computed里面
            var money=0;
...
/*        filters:{ //一定是filters因为有很多个过滤器
         currency:currency  //过滤器的原理是一个方法，给你一个值，然后进过处理得到另外一个值
         // 这里currency 是我们定义的一个函数，也能处理我们给的结果
        },*/
        mounted(){
          this.init()  //这里的是生命周期的很前面，这里把res.result赋值给this.cartList那么全局好用
        },
        components:{
          NavHeader,
          NavFooter,
        ...
        methods:{
          init(){
            ...
            //这个路由/users/cartList是执行哪个后台文件的路由，这个插件是通过promise形式的，这里get('路由',{参数})
          ...
          delCart(){
            axios.post("/users/cartDel",{productId:this.delItem.productId}).then((response)=>{
              let res = response.data;
              if(res.status == '0'){ //报头中res的status是 字符型的
          ...
          delCartConfirm(productId){
            this.delItem = productId; //这里要先把productId保存到全局，这样 delCart()才能从全局拿到
          ...
          editCart(flag,item){
            if(flag == 'add'){
              item.productNum++   //***虽然没有把item.productNum放在computed里面但是他这里把item.productNum当做是一个本地变量，这样也能提现出来，而且数字是真的和后台一样
            }else if(flag == 'minu'){
              if(item.productNum<=1){
                return;
              }
              item.productNum--  //把本地变量一个假值当做真的使用
            }else{
              item.checked = item.checked == '1'?'0':'1'
            }
            axios.post("/users/cartEdit",{
              productId:item.productId,   //把两个数据，传给后台
              productNum:item.productNum,
              checked:item.checked
            }).then((response)=>{
              let res = response.data; //这里是data 不是date ，只有在更新mongodb数据库的时候是update()
                //*******既然传回来的参数不能判断是+1 还是-1那么我们就从前端判断
                if(flag == 'minu'){
          ...
          toggleCheckAll(){
            //this.checkAllFlag = !this.checkAllFlag; 这里左边不能this.checkAllFlag，因为赋不进去，因为这里只要一点，上面compute就会计算，就会返回true，所以这里false就赋值不进去
            var flag = !this.checkAllFlag;
            this.cartList.forEach((item)=>{
              item.checked = flag?'1':'0'; //这个是在前端 把总数据cartList，里面数据遍历后把item.checked改成1
            })
            axios.post("/users/editCheckAll",{checkAll:flag}).then((response)=>{ 
            //这里传一个参数checkAll，它的值为this.checkAllFlag
              let res = response.data;
              if(res.status == '0'){
          ...
          checkOut(){
            if(this.checkedCount>0){ //checkedCount是个方法 也是要加this的
              this.$router.push({
                path:'/address'
              })
            }
