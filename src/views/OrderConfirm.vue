...
/*      filters:{    //如果在全局定义了filter 那么不能在子组件里面定义否者会报错
        currency:currency
      },*/ 
      methods:{
        init(){
          axios.get("/users/cartList").then((response)=>{
            ...
            this.cartList.forEach((item)=>{ //*******后台对传过来的json数据进行遍历
              if(item.checked =='1'){
        ...
        payMent(){
          ...
          axios.post("/users/payMent",{
            addressId:addressId,
            orderTotal:this.orderTotal //因为这里的orderTotal可以直接从全局拿所以不放到url里面传参了
          }).then((response)=>{
            let res = response.data; //请求成功之后就可以获取orderId的信息
            if(res.status == '0') {
              console.log("order create success");
              this.$router.push({ //如果要冲 url里面去参数的话那么要$route,如果页面跳转那么就是$router
                path:'/orderSuccess?orderId='+res.result.orderId //这个相当于前端跳转
              })
            }
          })
        }
