<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul> <!-- 只有当选中自己的时候才能选中，v-bind发生在checkIndex之后 -->
                  <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index,selectAddrId=item.addressId">  <!-- 这里是addressList要用数组的原因 -->
                    <dl> <!-- ***上面的@click事件只是用过点给变量赋值这个很实用 -->
                      <dt>{{item.userName}}</dt>
      ...
      <modal :mdShow="isMdShow" @close="closeModal"> <!-- 这里modal只要下面import了都能拿来用,之所以这里是@close是因为子组件通过emit中close来调用父组件的closeModal方法 -->
        <p slot="message">您确定要删除这条地址吗？</p>
        <div slot="btnGroup">
...
<script>
  ...
  export default{
      data(){ //组件内部实现 返回一个局部变量，因为这个组件要给被很多组件引用
          return{  //这里的data(){}必须是一个函数不能是一个对象或者变量
            addressList:[], //定义一个变量 类型是数组 
            limit:3,
            checkIndex:'',
            isMdShow:false,
            addressId:'',
          ...
          computed:{
            addressListFilter(){
              return this.addressList.slice(0,this.limit);//从0开始截取到limit 数据 
              //重新定义一个方法有一个好处是，它是全新的不会去 污染addressList里面的变量
            }
          },
          components:{
            NavHeader,
         ...
         methods:{
            init(){
              axios.get('/users/addressList').then((response)=>{
                ...
                this.addressList = res.result; //拿到数据以后就可以对数据进行渲染了
                //this.selectAddrId = res.result[0].addressId;
            ...
            expand(){
              if(this.limit == 3){ //多亏compute(计算)我们才能实现动态加载
            ...
            setDefault(addressId){
              ...
              //axios跨域请求相当于http请求，第二个参数是要传到后台的参数
                let res = response.data;
                if(res.status == '0'){
                  console.log("set default");
                  this.init(); // 重新调用了init() 想到与异步了
                }
